...
app.post('/api/chat',async(req,res)=>{
  ...
  try{
    ...
    if(call&&call.name==='create_order'){
      ...
      chatSessions.set(session,[...contents,candidate].slice(-12));return res.json({reply,order:result.success?result:null});
    }
    const reply=candidate?.parts?.map(p=>p.text||'').join('')||'আপনাকে কীভাবে সাহায্য করতে পারি?';chatSessions.set(session,[...contents,candidate].slice(-12));res.json({reply})
